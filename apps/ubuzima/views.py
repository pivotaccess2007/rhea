...
def match_pregs(req,diced,alllocs=False):
    rez = {}
    pst={}
    
    try:
        rez['created__gte']=diced['period']['start']
        rez['created__lte']=diced['period']['end']  
    except KeyError:
        pass      
    try:        
    ...
    try:
        ...
        pass
    try:
        rez['original__id']=int(req.REQUEST['location'])
    except KeyError:
        pass
    ...
    try:
        ...
        if uloc and uloc.location.type.name=='Health Centre':
            ...
            dst=uloc.location.id
            lox = LocationShorthand.objects.filter(district =dst )
            pst['report__location__in'] = [x.original for x in lox]
        elif uloc and uloc.location.type.name=='Province':
            prv=uloc.location.id  
            lox = LocationShorthand.objects.filter(province =prv )
            pst['report__location__in'] = [x.original for x in lox]
    ...
    if rez:
        pregs=PregStat.objects.filter(**rez)
    if pst:
        pregs=pregs.filter(**pst)
    return pregs
def match_stats(req, diced, alllocs = False):
    ...
    if rez:
        gs=GlobalStatistic.objects.filter(**rez)
    if pst:
        gs=gs.filter(**pst)
    return gs
...
def get_stats_track(req, filters):
    ...
    gs=match_stats(req, filters)
    track = {'births':len(reps.filter(type__name ='Birth')), 'pregnancy':len(reps.filter(type__name ='Pregnancy')),'anc':len(reps.filter(type__name ='ANC')),
            'childhealth':len(reps.filter(type__name ='Child Health')), 'risks': len(reps.filter(type__name ='Risk')),'matdeaths':len(gs.filter(key='md')),'chideaths':len(gs.filter(key='cd')),'newbdeaths':len(gs.filter(key='nd'))}
...
def default_period(req):
    ...
    return {'start':date.today() - timedelta(30), 'end':date.today()}#In production
...
def fetch_edd_info(req, diced, alllocs=False):
    ...
    try:
        rez['edd__gte']=diced['period']['start']
        rez['edd__lte']=diced['period']['end']  
    except KeyError:
        pass      
    try:
        if alllocs: raise KeyError
        loc = int(req.REQUEST['location'])
        rez['original__id'] = loc
    except KeyError:
        try:
            dst=int(req.REQUEST['district'])
            rez['ditrict__id'] = dst
            ...
            try:
                prv=int(req.REQUEST['province'])
                rez['province__id'] = prv
            except KeyError:    pass
    try:
        uloc=UserLocation.objects.get(user=req.user)
        if uloc and uloc.location.type.name=='Health Centre':
            loc=uloc.location.id
            pst['original__id'] = loc
        elif uloc and uloc.location.type.name=='District':   
            dst=uloc.location.id
            pst['ditrict__id'] = dst
        elif uloc and uloc.location.type.name=='Province':
            prv=uloc.location.id  
            pst['province__id'] = prv
...
def fetch_high_risky_preg(qryset):
    ans=qryset.filter(is_high_risk=True)    
    #ans=[]
...
def fetch_without_toilet(qryset):
    ans=qryset.filter(has_toilet=False)    
    #ans=[]
    #preg=qryset.filter(type=ReportType.objects.get(name='Pregnancy'))
    #for x in preg:
...
def fetch_without_hw(qryset):
    ans=qryset.filter(has_hand_wash=False)        
    #ans=[]
    #preg=qryset.filter(type=ReportType.objects.get(name='Pregnancy'))
    #for x in preg:
...
def get_important_stats(req, flts):
    rpt    = ReportType.objects.get(name = 'Birth')
    regula = match_stats(req, flts)
    reps=matching_reports(req, flts)
    qryset = regula.filter(report_type = rpt).select_related('report__patient')
    #stds, nstd = Gatherer(reps).distinguish()
    ans    = [
   {'label':'Expected deliveries',          'id':'expected',
   'number':len(fetch_edd_info(req,flts))},
   {'label':'Underweight Births',           'id':'underweight',
   'number':len(qryset.filter(field__value__lt = Decimal("2.5"), key='child_weight'))},
   {'label':'Delivered at Home',            'id':'home',
   'number':len(qryset.filter(key='ho'))},
   {'label':'Delivered at Health Facility', 'id':'facility',
   'number':len(qryset.filter(key__in=['hp','cl']))},
   {'label':'Delivered en route',           'id':'enroute',
	'number':len(qryset.filter(key='or'))},
    {'label':'Delivered Unknown',           'id':'unknown',
   'number':len(set( x.report for x in qryset.filter(unknown_loc =True)))}
   ]
    return ans
...
def risk_details(req,**flts):
    ...
    reps=match_stats(req, filters)
    lxn,crd = location_name(req), map_pointers(req,
            filters['location'], filters)
    for f in ftr:
        tf[f.key]={}
        try:
            ans=reps.filter(key=f.key)
...
def risk_stats(req,format,dat):
    ...
    rez=match_stats(req, filters).filter(key=f.key)
    lxn,crd = location_name(req), map_pointers(req,
            filters['location'], filters)
    ...
    if format == 'csv':
        ...
        wrt = csv.writer(htp, dialect = 'excel-tab')
        for r in rez:
            try: seq.append([r.report.id, r.report.reporter.connection().identity, r.report.location,
            r.report.patient,r.report.summary(), r.report.reporter.reporter_sups()])
            except Exception: continue
        wrt.writerows([heads]+seq)
        return htp
    else:
...
##New Version Statistics generator

@permission_required('ubuzima.can_view')
def view_global_stats(req, **flts):
    filters = {'period':default_period(req),
             'location':default_location(req),
             'province':default_province(req),
             'district':default_district(req)}
    track   = get_global_stats_track(req, filters)
    hindics = health_indicators(req, filters)
    stt = filters['period']['start']
    fin = filters['period']['end']
    lox, lxn, crd = 0, location_name(req), map_pointers(req,
    ...
    return render_to_response(req, 'ubuzima/stats.html',
       ...
       'start_date':date.strftime(filters['period']['start'], '%d.%m.%Y'),
         'end_date':date.strftime(filters['period']['end'], '%d.%m.%Y'),
           'coords':crd, 'location': lox, 'locationname':lxn,
           'chosen':the_chosen(req.REQUEST),
        'important':get_important_global_stats(req, filters),
          'targets':HealthTarget.objects.all(),
           'postqn':(req.get_full_path().split('?', 2) + [''])[1]})

def matching_global_stats(req, diced, alllocs = False):
    rez = {}
    pst = {}
    try:
        rez['created__gte'] = diced['period']['start']
    ...
    if rez:
        ans = GlobalStatistic.objects.filter(**rez).order_by("-created")
    else:
       ans = GlobalStatistic.objects.all().order_by("-created")
	
    if pst:
        ans = ans.filter(**pst).order_by("-created")
    return ans

def get_global_stats_track(req, filters):
    gs=matching_global_stats(req, filters)
    track = {'births':len(gs.filter(report_type__name="Birth").values('report').distinct()), 'pregnancy':len(gs.filter(report_type__name="Pregnancy").values('report').distinct()),'anc':len(gs.filter(report_type__name="ANC").values('report').distinct()),
            'childhealth':len(gs.filter(report_type__name="Child Health").values('report').distinct()), 'risks': len(gs.filter(report_type__name="Risk").values('report').distinct()),'matdeaths':len(gs.filter(key='md')),'chideaths':len(gs.filter(key='cd')),'newbdeaths':len(gs.filter(key='nd'))}
    
    return track


def get_important_global_stats(req, flts):
    rpt    = ReportType.objects.get(name = 'Birth')
    regula = match_stats(req, flts)
    reps=matching_global_stats(req, flts)
    qryset = regula.filter(report_type = rpt).select_related('report__patient')
    #stds, nstd = Gatherer(reps).distinguish()
    ans    = [
   {'label':'Expected deliveries',          'id':'expected',
