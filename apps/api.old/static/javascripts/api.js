...
function checkMsgs() {
	if ($('#patid').val().length > 0) {
		req = url + $('#patid').val() + "/json_resp";
		$.getJSON(
			req,
			function (response) { if (response) {
				snippet = '<tr class="out"><td class="phone">' + response + '</td><td class="dir">&raquo;</td><td class="msg">' + response + '</td><td class="info">' + response + ' characters</td></tr>';
				$('#log').append(snippet);
				fixClasses();
				$('div.tester').scrollTo('#log tr:last', 800);
			}}
