...
class RheaNotification(object):
	...
	def createMSH(self, root, report):
		...
		msh_4_hd_1 = SubElement(msh_4, "HD.1");msh_4_hd_1.text = code_to_fosa(report.location.code)
	...
	def createPV1(self, root, report):
		...
		pv1_7_xcn_1 = SubElement(pv1_7, "XCN.1");pv1_7_xcn_1.text = "3525410" #report.reporter.alias #remember to change this in production
	...
	def createDOB(self, root, index, report):
		...
		dob = "%04d%02d%02d%02d%02d%02d"%(report.date.year,report.date.month,report.date.day,00,00,00)
	...
	def createRiskLocationSex(self, root, index, field):
		...
		p, concept = None, None
		#print field
		if field.type.category.name == 'Risk':
			try:
		...
		elif field.type.category.name == 'Death':
			try:
	...
	def createOBX(self, root, report):
		...
		for f in fields:
			...
			if f.type.key in ['mother_weight', 'child_weight', 'child_number']:	self.createWeightChildNumber(SubElement(root, "ORU_R01.OBSERVATION")\
																, fields.index(f), f)
			elif f.type.category.name in ['Risk','Death', 'Movement'] or f.type.key in ['bo', 'gi']:	self.createRiskLocationSex(SubElement(root,\
				...
				if len(main_nodes) == 2:
					try:
						...
						if msh.tagName == 'MSH' and result.tagName == 'ORU_R01.PATIENT_RESULT':
							...
							#print msh, result, msg, sent
							print sent
							data = True
							#if sent != True:	data = False ##Rember to uncomment this line after modem and credit check
							#else:	data = True 		     ##Rember to uncomment this line after modem and credit check
						else:	data = None
					except:	data = None
				else:	data = None
		response = self.response_to_xml(data)
	...
	def sendNotification(self, report):
		notif, res = None, None
		if report.type.name in ['Birth', 'Risk'] :
			message = self.createOutgoingNotification(report)
			notif = Notification(not_type = NotificationType.objects.get(name = report.type.name), message = message, report = report)
		if report.fields.filter( type__category__name = 'Death').exists():
			message = self.createOutgoingNotification(report)
			notif = Notification(not_type = NotificationType.objects.get(name = "Death"), message = message, report = report)
			
		try:
			report.patient.national_id = "123456789"#Remove this line in production
			url = "/ws/rest/v1/patient/NID-%s/encounters?notificationType=%s" % (report.patient.national_id, self.getMessageHeader(report))
			url = "https://%s:%s%s"%(conf["host"], conf["port"], url)
			request = urllib2.Request(url, notif.message, headers = {'Content-Type': 'application/xml'})
			
			base64string = base64.encodestring('%s:%s' % (conf["user"], conf["pass"]))[:-1]
			request.add_header("Authorization", "Basic %s" % base64string)
			#print url, notif.message, request
			res = get_rhea_response(request)
			#print "Patient: %s, Response Status: %s" % (report.patient, res)
		except Exception, e:
			#print e
			pass
	...
	def import_concepts(self, filepath = "apps/api/concepts.xls", sheetname = "concepts", startrow = 1, maxrow = 48, namecol = 0, answercol = 1,\
