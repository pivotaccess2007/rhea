...
from ubuzima.management.commands.checkreminders import *
...
def index(req):
    ...
    if req.REQUEST.has_key('csv'):
        heads=['ReportID','Telephone','NationalID','LastSeen','Village','Language','Location','Supervisors']
        htp = HttpResponse()
        htp['Content-Type'] = 'text/csv; encoding=%s' % (getdefaultencoding(),)
        try:
            wrt = csv.writer(htp, dialect = 'excel-tab')
            wrt.writerows([heads]+[[r.id, r.connection().identity,r.alias,r.last_seen(),r.location,[ str(y.connection().identity) for y in r.reporter_sups()]] for r in repos])
...
def view_active_reporters_csv(req,**flts):
    ...
    wrt.writerows([[x.pk,x.connection().identity,x.alias,x.location,x.last_seen(),[ str(y.connection().identity) for y in x.reporter_sups()] ] for x in active_reporters(req,rez)])
    return rsp
...
def view_inactive_reporters_csv(req,**flts):
    ...
    rsp['Content-Type'] = 'text/csv; encoding=%s' % (getdefaultencoding(),)
    wrt = csv.writer(rsp, dialect = 'excel-tab')
    wrt.writerows([[x.pk,x.connection().identity,x.alias,x.location,x.last_seen(),[ str(y.connection().identity) for y in x.reporter_sups()] ] for x in inactive_reporters(req,rez)])
    return rsp
...
def error_list_csv(req,**flts):
    ...
    rsp['Content-Type'] = 'text/csv; encoding=%s' % (getdefaultencoding(),)
    wrt = csv.writer(rsp, dialect = 'excel-tab')
    wrt.writerows([[r.errby.connection().identity, r.errmsg, r.errby.location, r.created] for r in errs.filter(**ps)])
    return rsp
        ...
        if req.method == 'POST':
            ...
            if form.is_valid():
                ...
                for row_index in range(sheet.nrows):
                        ...
                        reporter=None
                        row_num     = row_index
                        if row_num < 1 or len(parse_alias(sheet.cell(row_index,0).value))!= 16: continue
                        try:
                                #id = sheet.cell(row_index,0).value
                                #print row_num, parse_alias(sheet.cell(row_index,0).value)
                                reporter,created=Reporter.objects.get_or_create(alias=parse_alias(sheet.cell(row_index,0).value)) 
                                reporter.location = Location.objects.get(code=fosa_to_code(str(int(sheet.cell(row_index,3).value))))
                                reporter.first_name=parse_name(sheet.cell(row_index,1).value)[0]
                                reporter.last_name=parse_name(sheet.cell(row_index,2).value)[1]
                                ...
                                try:
                                    ...
                                    p=PersistantConnection(backend=PersistantBackend.objects.get(title="kannel"),identity=parse_phone_number(sheet.cell(row_index,7).value),reporter=reporter,last_seen=datetime.datetime.now())
                                    p.save()
                                    #print reporter.connections.get(backend__title="kannel")
                                    alert_to_start(reporter)#alert the reporter to start reporting to 1110 line
